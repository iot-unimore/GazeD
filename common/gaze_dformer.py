...
class GazeTransformer(ModelMixin, nn.Module):
    def __init__(self, config=None, num_frame=1, num_joints=17, num_queries = 20, in_chans=2, 
        ...
        self.box_embed = nn.Linear(in_chans,embed_dim_ratio)
        ...
        self.project_objects = nn.Linear(256 ,768)

        self.object_blocks = nn.MultiheadAttention(embed_dim_ratio*6,num_heads=8)
        
        self.Spatial_pos_embed = nn.Parameter(torch.zeros(1, self.levels, num_joints, embed_dim_ratio))
        ...
        self.project_eye = nn.Linear(2,256)
    ...
    def forward(self, keypoints_2d, ref, noisy_keypoints_3d, features_list, t, objects, eye_midpoint):
        # concatenate 2D keypoints and 3D noisy keypoints and embed them
        

        embed = objects.unsqueeze(0).unsqueeze(0)
        

        
        if len(noisy_keypoints_3d.shape) > 4:
            #keypoints_2d = torch.cat((keypoints_2d,torch.ones((keypoints_2d.shape[0],1,1,2), device="cuda:0")),axis=2)
            ...
            objects = embed.repeat(1,h,1,1,1)
            objects = rearrange(objects,'l h b p c -> p (b h) (l c)', b=b, h=h)        
            
            x_3d_noisy = rearrange(noisy_keypoints_3d, 'b h f p c -> (b h f) p c')
            #x_3d_noisy = x_3d_noisy[:,:-1,:]
            ...
            x = self.coord_embed(x)
            objects = rearrange(embed,'l h b p c -> p (b h) (l c)', b=b, h=1)
        ...
        x = rearrange(x, 'b j c -> j b c')
    
        objects = self.project_objects(objects)
        
        objects,_ = self.object_blocks(objects,x,x)
        
        gaze = torch.mean(objects,dim=0, keepdim=True)
        
        x = torch.cat((x[:-1],gaze), dim=0)
        
        x = rearrange(x, 'j b c -> b j c')
